...
function adminOverview(){
  ...
  const gateways=db.gateways.filter(g=>!simulated(g)),hangers=db.hangers.filter(h=>!simulated(h)),garments=db.garments;
  const users=db.users.map(user=>{
    ...
    return {...userPublic(user),gatewayCount:ownedGateways.length,hangerCount:ownedHangers.length,garmentCount:ownedGarments.length,gateways:ownedGateways.map(gateway=>{
      ...
      return {gatewayId:gateway.gatewayId,gatewayNumber:gateway.gatewayNumber||null,name:gateway.name,customName:gateway.customName||'',state:online(gateway)?'ONLINE':'OFFLINE',lastSeen:gateway.lastSeen||null,hangerCount:gatewayHangers.length,hangers:gatewayHangers.map(hanger=>{
        const garment=ownedGarments.find(item=>item.tagUid===hanger.tagUid&&hanger.state==='PRESENT');
        return {hangerId:hanger.hangerId,hangerNumber:hanger.hangerNumber||null,name:hanger.alias,customName:hanger.customName||'',gatewayId:hanger.gatewayId,state:online(hanger)?'ONLINE':'OFFLINE',reportedState:hanger.reportedState||hanger.state||'UNKNOWN',nfcStatus:Number(hanger.errorFlags||0)>0?'점검 필요':hanger.firmwareVersion&&hanger.firmwareVersion!=='unknown'?'정상':'확인 중',tagDetected:!!hanger.tagUid,garmentName:garment?.name||'',lastSeen:hanger.lastSeen||null};
      })};
    })};
  });
  ...
  return {totals:{users:users.length,gateways:gateways.length,hangers:hangers.length,garments:garments.length,onlineGateways,offlineGateways:gateways.length-onlineGateways,onlineHangers,offlineHangers:hangers.length-onlineHangers},users,system:{backend:{ready:isReady,storage:storage.mode},imageProcessing:{configured:cloudImageService.configured(),ready:imageStates.ready||0,processing:imageStates.processing||0,pending:imageStates.pending||0,failed:imageStates.failed||0},recentDeviceEvents}};
}
