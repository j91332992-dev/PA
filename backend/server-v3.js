...
function discoveredHangers(wid){const ownedGatewayIds=new Set(db.gateways.filter(g=>g.wardrobeId===wid).map(g=>g.gatewayId)),cutoff=Date.now()-120000;return db.hangers.filter(h=>!h.wardrobeId&&ownedGatewayIds.has(h.gatewayId)&&Date.parse(h.lastSeen||0)>=cutoff&&(SIM||!simulated(h))).map(h=>({hangerId:h.hangerId,alias:h.alias||'',gatewayId:h.gatewayId,createdAt:h.createdAt||null,reportedState:h.reportedState||h.state||'UNKNOWN',state:h.state||'UNKNOWN',tagUid:h.tagUid||null,lastSeen:h.lastSeen||null,channel:h.channel||0,errorFlags:h.errorFlags||0,firmwareVersion:h.firmwareVersion||'unknown'}))}
