...
// A Gateway uploads C6 status over HTTPS sequentially.  With two physical
// hangers a healthy Render request can take longer than one 750ms liveness
// window, so that value produced false OFFLINE/PRESENT flapping. NFC EMPTY
// remains an immediate event; this timeout only detects a truly lost link.
const OFFLINE=Number(process.env.OFFLINE_TIMEOUT_MS||30000),HANGER_OFFLINE=Number(process.env.HANGER_OFFLINE_TIMEOUT_MS||2500),CMD_TIMEOUT=Number(process.env.COMMAND_TIMEOUT_MS||60000),SIM=process.env.SIMULATION_ENABLED==='true';
...
function statusBatch(x){
  const entries=Array.isArray(x?.statuses)?x.statuses:[];
  if(!entries.length||entries.length>16)throw error(400,'옷걸이 상태 묶음은 1~16개여야 합니다.');
  const common={gatewayId:x.gatewayId,channel:x.channel,gatewayFirmwareVersion:x.gatewayFirmwareVersion};
  return entries.map(entry=>status({...common,...entry}));
}
...
function command(targets,user,duration=0,kind='LED_BLINK'){const w=wardrobeFor(user),commandName=String(kind).toUpperCase()==='LED_OFF'?'LED_OFF':'LED_BLINK';targets=[...new Set((targets||[]).map(x=>String(x).toUpperCase()))];if(!targets.length||targets.length>16)throw error(400,'대상은 1~16개여야 합니다.');const hangers=targets.map(t=>{const hanger=db.hangers.find(h=>h.hangerId===t&&h.wardrobeId===w.id);if(!hanger)throw error(404,`내 옷장에 없는 옷걸이: ${t}`);return hanger});if(commandName==='LED_BLINK'&&hangers.some(hanger=>hanger.state!=='PRESENT'||!hanger.tagUid))throw error(409,'옷장 안에서 태그가 인식된 옷걸이만 LED 찾기를 할 수 있습니다.');cancelActiveCommands(targets,w.id,'SUPERSEDED');const c={id:id('cmd'),numericId:crypto.randomInt(1,2147483647),command:commandName,targets,durationMs:Math.max(0,Math.min(120000,Number(duration)||0)),status:'QUEUED',requestedBy:user.id,wardrobeId:w.id,createdAt:now(),expiresAt:new Date(Date.now()+CMD_TIMEOUT).toISOString(),acknowledgements:{}};db.commands.unshift(c);console.log(`[COMMAND] QUEUED id=${c.numericId} type=${c.command} targets=${targets.join(',')}`);emit('command.queued',c,'info',w.id);return c}
...
if(process.env.DISABLE_BACKGROUND_TASKS!=='true')setInterval(async()=>{let dirty=false;for(const h of db.hangers)if(h.state!=='OFFLINE'&&Date.parse(h.lastSeen||0)<Date.now()-HANGER_OFFLINE){h.reportedState=h.state='OFFLINE';const cancelled=h.wardrobeId?cancelActiveCommands([h.hangerId],h.wardrobeId,'ESPNOW_LINK_LOST'):[];emit('hanger.offline',h,'warning',h.wardrobeId);if(cancelled.length)emit('command.cancelled',{hangerId:h.hangerId,targets:[h.hangerId],reason:'ESPNOW_LINK_LOST',commandCount:cancelled.length},'warning',h.wardrobeId);dirty=true}for(const c of db.commands)if(!['ACKED','TIMEOUT','CANCELLED'].includes(c.status)&&Date.parse(c.expiresAt)<Date.now()){c.status='TIMEOUT';emit('command.timeout',c,'warning',c.wardrobeId);dirty=true}if(dirty){reconcile();await save()}},100).unref();
