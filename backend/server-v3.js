...
if(p==='/api/gateway/commands'){needDevice(req);const gatewayId=String(req.headers['x-gateway-id']||'').toUpperCase(),gateway=db.gateways.find(g=>g.gatewayId===gatewayId),wid=gatewayOwner(gatewayId),resets=[];if(gateway?.resetPending==='FACTORY_RESET'){resets.push({numericId:0,command:'FACTORY_RESET',targets:[],durationMs:0});gateway.resetPending=null;}for(const hangerId of gateway?.pendingHangerResets||[])resets.push({numericId:0,command:'UNPAIR',targets:[hangerId],durationMs:0});if(gateway)gateway.pendingHangerResets=[];const cs=db.commands.filter(c=>wid&&c.wardrobeId===wid&&['QUEUED','SENT','PARTIAL'].includes(c.status)&&Date.parse(c.expiresAt)>Date.now()&&c.targets.some(t=>db.hangers.some(h=>h.hangerId===t&&h.gatewayId===gatewayId)));const pairs=[];for(const c of cs){const targets=c.targets.filter(t=>db.hangers.some(h=>h.hangerId===t&&h.gatewayId===gatewayId&&h.wardrobeId===wid));if(targets.length)pairs.push({numericId:0,command:'PAIR',targets,durationMs:0});c.status='SENT';c.sentAt=now();console.log(`[COMMAND] SENT id=${c.numericId} gateway=${gatewayId} targets=${c.targets.join(',')}`)}json(res,200,{commands:[...resets,...pairs,...cs]});if(resets.length||cs.length)save().catch(e=>console.error('[COMMAND] command status save failed',e.message));return}
