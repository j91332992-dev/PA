...
void fetchCommands(){String out;if(!request("/api/gateway/commands","GET","",out))return;JsonDocument doc;if(deserializeJson(doc,out))return;for(JsonObject c:doc["commands"].as<JsonArray>()){sw::Packet p;p.type=sw::Type::COMMAND;strlcpy(p.gatewayId,gateway.c_str(),sizeof p.gatewayId);p.sequence=++sequence;p.commandId=c["numericId"]|0;p.command=sw::Command::LED_BLINK;p.durationMs=c["durationMs"]|8000;JsonArray targets=c["targets"];p.targetCount=min<size_t>(targets.size(),sw::MAX_TARGETS);for(uint8_t i=0;i<p.targetCount;i++)p.targetIds[i]=sw::idCode(targets[i]|"");for(uint8_t i=0;i<3;i++){send(p);delay(25+esp_random()%50);}Serial.printf("[COMMAND] %lu targets=%u\n",p.commandId,p.targetCount);}}
